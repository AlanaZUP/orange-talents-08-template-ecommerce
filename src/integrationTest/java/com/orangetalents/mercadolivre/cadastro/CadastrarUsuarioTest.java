...
public class CadastrarUsuarioTest {
    ...
    @Test
    public void retornaBadRequestQuandoRecebeLoginESenhaInvalidos() throws Exception {
        UsuarioRequest usuarioRequest = new UsuarioRequest("loginErrado", "12345");
        mockMvc.perform(post("/cadastro")
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(gson.toJson(usuarioRequest)))
                .andExpect(status().isBadRequest())
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].campo").value("login"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[1].campo").value("senha"));
    }
